function [sig_elecs_by_subj,nonsig_elecs_by_subj,sig_count,nonsig_count,...
    find_sig_elecs_header,cfc_all_subj] = find_sig_elecs_cluster_pixel_combo(cfc_all_subj,subjs,plot_elec)
...
for s=1:length(subjs)   
    ...
    subj_num_clust = {};
    subj_clust_idx = {};
    subj_adj_pvals = {}; %adjusted pvals from fdr correction for each electrode
    subj_clust_amps = {}; %amplitude freqs of largest cluster for each electrode
    subj_clust_phases = {}; %amplitude freqs of largest cluster for each electrode
    subj_clust_mats = {}; %matrix of normalized plvs for most significant clusters
    subj_clust_zstats = {}; %matrix of pvals corresponding to pixels in most significant clusters
    subj_clust_size = {}; %proportion of cluster pvalues <= 0.05 (for significant elecs should always >= 0.95)
    ...
    for e=1:data.n_elecs
        ...
        %get pvalues for fdr correction
        ...
        %get significant pixels only after fdr correction
        ...
        %cluster with bwlabel
        clusters = bwlabel(adj_plv,4);
        num_clust = max(clusters,[],'all');
        
        for c=1:length(num_clust)
            clust_idx = find(clusters==c);
            clust_size = numel(clust_idx);
            clust_mat = adj_plv(clust_idx);
            clust_z_stat = sum(clust_mat);
        end 
        if num_clust == 0
