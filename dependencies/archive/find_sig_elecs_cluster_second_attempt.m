...
for s=1:length(subjs)   
    ...
    for e=1:data.n_elecs
        ...
        elseif (max(contourTable.Level) > 2 && max(contourTable.Level)<=5) %for clusters that have max std greater than 2 but less than/= 5
            max_z = (max(contourTable.Level)-1); %setting max to max cluster + second largest cluster (cutoff is all levels >= max-1)
            max_level = contourTable.Level >= max_z; %extracts all values of highest level clusters 
        else % some elecs have massive max zscore values
            max_z = 5; %if max zscore goes up to 5 std above mean - all levels above that are counted in significant contour 
            max_level = contourTable.Level>=max_z; %extract info from all significant contours (some elecs have max std up to 40)
        end 
        ...
        if sum(max_level) == 0 %some electrodes have no significant clusters (max level <2)
            ...
            sig_amp_idx_corrected = []; % no significant amps 
            ...
            %step 4:
                % to be considered a cluster - must have >= 2 amps for every freq band
                % must correct cluster amp indices (remove noise)
            
            sig_amp_idx_corrected = zeros(size(sig_amp_idx)); %empty amp index vec
            
            % for each amp freq band of interest - determine if there are 2
            % or more freqs from band in cluster 
            
            % PROBLEM - theta only has one amp value represented, alpha
            % only has 2
            delta_theta = [2 8]; %combined delta/theta band
            delta_theta_idx = sig_amp_idx(discretize(sig_amps,delta_theta)==1); %determine if clust indices are sequential - if not then cannot be inluded in cluster
            if length(delta_theta_idx) >1 %for each freq band, must be at least 2 sequential amp freq indices to be considered part of the cluster
                sig_amp_idx_corrected(delta_theta_idx,1) = delta_theta_idx; %add amp freqs to corrected amp index if meet criteria
            end 
            
            alpha = [8 13]; %alpha band
            alpha_idx = sig_amp_idx(discretize(sig_amps,alpha)==1); %determine if clust indices are sequential - if not then cannot be inluded in cluster
            if length(alpha_idx) >=2 %for each freq band, must be at least 2 sequential amp freq indices to be considered part of the cluster
                sig_amp_idx_corrected(alpha_idx,1) = alpha_idx; %add amp freqs to corrected amp index if meet criteria
            end 
            
            beta = [13 30]; %beta band
            beta_idx = sig_amp_idx(discretize(sig_amps,beta)==1); %determine if clust indices are sequential - if not then cannot be inluded in cluster
            if length(beta_idx) >=2
                sig_amp_idx_corrected(beta_idx,1) = beta_idx; %add amp freqs to corrected amp index if meet criteria
            end 
            
            gamma = [30 70]; %gamma band
            gamma_idx = sig_amp_idx(discretize(sig_amps,gamma)==1); %determine if clust indices are sequential - if not then cannot be inluded in cluster
            if length(gamma_idx) >=3 %for each freq band, must be at least 2 sequential amp freq indices to be considered part of the cluster
                sig_amp_idx_corrected(gamma_idx,1) = gamma_idx; %add amp freqs to corrected amp index if meet criteria
            end 
            
            hga = [70 200]; %hga band
            hga_idx = sig_amp_idx(discretize(sig_amps,hga)==1); %determine if clust indices are sequential - if not then cannot be inluded in cluster
            if length(hga_idx) >=4 %hga is broadband - cutoff higher (sig across 5*cutoff freqs) to be considered part of the cluster
                sig_amp_idx_corrected(hga_idx,1) = hga_idx; %add amp freqs to corrected amp index if meet criteria
            end
            
            sig_amp_idx_corrected = nonzeros(sig_amp_idx_corrected); %remove zeros from corrected amp idx - corresponds to freq idx that didn't meet criteria 
%             sig_amp_corrected = amp_f_array(sig_amp_idx'); %get actual freq values from corrected significant amp freq indices
            

            %step 5: 
            
            if length(sig_amp_idx_corrected) <2 %making sure clusters with only one freq are eliminated (should already be gone)
                %define null matrices for nonsig data structure (sig_amp_idx_corrected & sig_phase_ix are still defined)
                cluster_mat = []; % no significant clusters
                cluster_pvals = []; % p=1 (placeholder to indicate not significant)
                cluster_sig_pixel_prop = 0; % no significant pixels within cluster 

            else 
                cluster_mat = plv(sig_amp_idx_corrected,sig_phase_idx); %extract norm plv matrix of corrected cluster
                cluster_pvals = adj_p(sig_amp_idx_corrected,sig_phase_idx); %get corrected pvalues of every pixel in cluster
                ...
                if numel(cluster_mat) <4 % some clusters are still toooooo small (4 or more total pixels in clust required)
                    %define null matrices for nonsig data structure (sig_amp_idx_corrected & sig_phase_ix are still defined)
                    cluster_mat = []; % no significant clusters
                    cluster_pvals = []; % p=1 (placeholder to indicate not significant)
                    cluster_sig_pixel_prop = 0; % no significant pixels within cluster
        ...
        if cluster_sig_pixel_prop >=0.95 % alpha = 0.05, if prop of sig cluster surpasses false positive rate - clust = significant 
            ...
            subj_sig_clust_amps(subj_count,:) = {sig_amp_idx_corrected}; % amp freq index (corrected!)
            ...
            subj_nonsig_clust_amps(nonsig_subj_count,:) = {sig_amp_idx_corrected}; %amp freqs of largest cluster (zero if no clusters)
