function [plv_matrix] = plot_phase_amp_ranges(SUBID, meta_ID, cond_string, e_mat, ...
                                            start_window, ...
                                            end_window, ...
...
if ~exist([pth_anal 'plv_graphs/'],'dir') % checks if the appropriate subfolder for this study/block has been created in 'analysis'....
    mkdir([pth_anal 'plv_graphs/']);
end
...
if run_surrogate %if we are creating a surrogate distribution
    if ~exist([pth_anal 'plv_graphs/' cond_string '/cond_permute_rand_byfreq'],'dir') % checks if the appropriate subfolder for this study/block has been created in 'analysis'....
        mkdir([pth_anal 'plv_graphs/' cond_string '/cond_permute_rand_byfreq']);
    end
    
else
    if ~exist([pth_anal 'plv_graphs/' cond_string '/cond_permute'],'dir') % checks if the appropriate subfolder for this study/block has been created in 'analysis'....
        mkdir([pth_anal 'plv_graphs/' cond_string '/cond_permute']);
    end
...
% %this chunks out data for a particular condition-- this should go AFTER
% filtering? (note sure if this makes a difference?)
% [e_signal, sp] = get_session_data(e_mat,srate,subject,cond_string, ...
...
%for e = 1:size(elecs,2)

%e_signal = raw_signal(elecs(e),:);
...
if run_surrogate
    ...
    print('-dpdf', [pth_anal 'plv_graphs/' cond_string '/cond_permute_rand_byfreq/' 'PLV_rand_byfreq_hi_' elec '_lo_' elec]);
else
    print('-dpdf', [pth_anal 'plv_graphs/' cond_string '/PLV_hi_' elec '_lo_' elec]);
