function [plv_matrix] = plot_phase_amp_ranges_AF(data_path, save_path, SUBID, ...
    cond_string, elec_num, srate, ...
    start_window, ...
    end_window, ...
    run_surrogate, ...
    varargin)
...
%
%     data_path         - Local path for data location. taken in as a
%                         string
%     save_path         - Local path to save figs. taken in as a
%                         string
%
%     SUBID             - Subject ID (ex- 's02'). taken in as a
%                         string
%
%     elec_num          - single elec num to load
%
...
%     Modified by Alexandra Fink 09/14/2023


%% Things to fix: 
%required helper functions - get_signal_parameters, get_session_data,
%make_chirplet,filter_with_chirplet, pval_from_gamma_dist


%helper functions with hard coded paths - get_session_data
...
pth_data = [data_path SUBID]; 
pth_img = [save_path SUBID]; 
load(strcat(pth_data,'/',SUBID,'_signal.mat')) %s13 signal_ds
load(strcat(pth_data,'/',SUBID,'_subj_globals.mat'))


e_signal = signal(elec_num,:);
%cd(pth_anal);

if ~exist([pth_img '_plv_graphs/'],'dir') % checks if the appropriate subfolder for this study/block has been created in 'analysis'....
    mkdir([pth_img '_plv_graphs/']);
end

if run_surrogate %if we are creating a surrogate distribution
    if ~exist([pth_img '_plv_graphs/' cond_string '/cond_permute_rand_byfreq'],'dir') % checks if the appropriate subfolder for this study/block has been created in 'analysis'....
        mkdir([pth_img '_plv_graphs/' cond_string '/cond_permute_rand_byfreq']);
    end
    
else
    if ~exist([pth_img '_plv_graphs/' cond_string '/cond_permute'],'dir') % checks if the appropriate subfolder for this study/block has been created in 'analysis'....
        mkdir([pth_img '_plv_graphs/' cond_string '/cond_permute']);
    end
...
for i_phase = 1:n_phase_freqs %for each frequency of the phase data
    ...
    for i_amp = 1:n_amp_freqs %for each frequency of the amplitude data
        ...
        [chunked_low_phase chunked_high_phase] = get_session_data_AF(SUBID, pth_data,...
...
% save PLV matrix 
save([pth_img '_plv_graphs/' cond_string '/cond_permute_rand_byfreq/' SUBID '_plv_matrix_real_' elec], 'plv_matrix_real');
...
if run_surrogate
    ...
    save([pth_img '_plv_graphs/' cond_string '/cond_permute_rand_byfreq/' SUBID '_xvals_all_dist_' elec], 'x_vals_all','contours');
    save([pth_img '_plv_graphs/' cond_string '/cond_permute_rand_byfreq/' SUBID '_perm_plv_matrix_surr' elec], 'plv_matrix_surr');
...
if run_surrogate
    print('-dpdf', [pth_img '_plv_graphs/' cond_string '/cond_permute_rand_byfreq/' SUBID '_PLV_rand_byfreq_hi_e' elec '_lo_e' elec]);
else
    print('-dpdf', [pth_img '_plv_graphs/' cond_string '/' SUBID '_PLV_hi_e' elec '_lo_e' elec]);
