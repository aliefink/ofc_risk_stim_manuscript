...
%
%   Written by Sara Szczepanski 6/2012, from scripts modified by Kris
%   Andersen
%
%   Modified by Ignacio Saez 2/2015
%
%   Questions:
%
...
p_vals = [0.5 0.1 0.05 0.01 0.005 0.001 0.0005]; % Desired p-values for plotting
...
x_vals_all = nan(size(surr_matrix,1),size(surr_matrix,2),length(p_vals)); %initialize a matrix that has the same number of amp frequencies & phase frequencies and three
...
for a = 1:size(surr_matrix,1) %for each amplitude frequency value in the surrogate matrix
    ...
    for p = 1:size(surr_matrix,2) %for each phase frequency value in the surrogate matrix
        ...
        % Fit surrogate PLV values to a gamma distribution- do this for
        % each pair of frequencies (each pair of frequencies has its own
        % distribution). gamma_params is a two element vector with (1) =
        % gamma_shape' and (2) = gamma_scale'
...
x_vals_6 = x_vals_all(:,:,6); %all x values associated with p value of 0.001
x_vals_7 = x_vals_all(:,:,7); %all x values associated with p value of 0.0005


% Choose which distribution to use to calculate p values: Find the
% distribution with the largest x value (the most conservative estimate -
% across all frequencies and timepoints) and use these values as contours
% in the graph (to mark significance boundaries).

% Sara's NOTE: I think I can do it this way, since the largest x value for 
% p = 0.1 will come from the same distribution as the largest x value for 
% p = 0.05, etc...
...
max_x_val_6 = max(x_vals_6(:));
max_x_val_7 = max(x_vals_7(:));

x_vals = [max_x_val_1 max_x_val_2 max_x_val_3 max_x_val_4 max_x_val_5, max_x_val_6, max_x_val_7]; % x vals = values to use as significance boundaries in contourf.m
...
for p_val_n = 1:length(p_vals)
  fprintf('\t\t%2.2f corresponds to p=%1.4f\n', x_vals(p_val_n), p_vals(p_val_n));
end
fprintf('\n');
